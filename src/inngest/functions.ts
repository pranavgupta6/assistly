  ...
  async ({ event , step }) => {
    ...
    const codeAgent = createAgent<AgentState>({
      ...
      tools:[
        createTool({
          name: "terminal",
          description : "use the terminal to run comands",
          parameters : z.object({
            command : z.string(),
          }),
      ...
      lifecycle : {
        onResponse : async ({ result ,  network }) => {
          const lastAssistanMessageText = lastAssistantTextMessageContent(result);

          if (lastAssistanMessageText && network){
            if(lastAssistanMessageText.includes("<task_summary>")){
              network.state.data.summary = lastAssistanMessageText;
            }
          }
